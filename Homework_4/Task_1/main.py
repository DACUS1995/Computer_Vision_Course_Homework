...
def detect_lines(image, use_original=False):
	output_image = np.copy(image) if use_original else None
	...
	for label in range(1, num_labels):
		output_image, lines_eq, lines_points, lines_class = RANSAC.search(labeled_image, label, output_image=output_image, original_image=None, total_classes_number=num_labels)
